...
public class CMD_Loader
{
    ...
    public static void scan()
    {
        ...
        COMMAND_LIST.stream().map((commandInfo) -> new PC_DynamicCommand(commandInfo)).forEach((dynamicCommand) ->
        {
            ...
            commandMap.register(UpToDate.plugin.getDescription().getName(), dynamicCommand);
        });
    }
    ...
    {
        final Object commandMap = getField(Bukkit.getServer().getPluginManager(), "UpToDate");
        if (commandMap != null)
        {
    ...
    {
        Object knownCommands = getField(commandMap, "UpToDate");
        if (knownCommands != null)
        {
    ...
    private static List<PC_CommandInfo> getCommands()
    {
        ...
        try
        {
            CodeSource codeSource = UpToDate.class.getProtectionDomain().getCodeSource();
            if (codeSource != null)
            {
